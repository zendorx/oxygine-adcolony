...
DECLARE_SMART(AdcolonyDialog, spAdcolonyDialog);
class AdcolonyDialog : public Actor
{
public:
    enum
    {
...
//Json::Value _adcolony(Json::objectValue);

/*void adcolonySimulatorLogin()
{
    spAdcolonyDialog dialog = new AdcolonyDialog;
    ...
    dialog->_btnOk->addClickListener([ = ](Event * e)
    {
        ...
        adcolony::internal::loginResult(true);
    });
...
void adcolonySimulatorInit()
{
    log::messageln("Adcolony Simulator Init");


    /*file::buffer bf;
    file::read(".adcolony", bf, ep_ignore_error);

    if (!bf.empty())
    {
        Json::Reader reader;
        reader.parse((char*)&bf.front(), (char*)&bf.front() + bf.size(), _adcolony, false);
    }*/
}
